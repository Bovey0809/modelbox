...
main() {
    ...
    mv ${TARGET_DIR}/src/graph/yolo26n_arc770.toml.in ${TARGET_DIR}/src/graph/yolo26n_arc770.toml
    sed -i "s#@DEMO_YOLO26N_ARC770_FLOWUNIT_DIR@#@APPLICATION_PATH@/flowunit#g" ${TARGET_DIR}/src/graph/yolo26n_arc770.toml
    if [ $? -ne 0 ]; then
        echo "change graph path failed."
        return 1
    fi

    sed -i "s#@DEMO_VIDEO_DIR@#@PROJECT_PATH@/src/graph#g" ${TARGET_DIR}/src/graph/yolo26n_arc770.toml
    if [ $? -ne 0 ]; then
