...
- **EOF deadlock at end of stream.** With the v1 wiring (graph commit `7f18dba`, smoke `a9c0f2c`), the pipeline produces ~207 of 220 input frames into the output MP4 and then hangs in `Flow::Wait`. The bypass `videodecoder:out_video_frame -> tracknet_post:source_frame` and the STREAM-typed `tracknet_frame_stacker` reach EOF asymmetrically; `tracknet_post`'s two-input match never finalizes and the encoder is never told to flush. Switching the stacker to `NORMAL` removes the hang but produces zero output (DataPre/SetPrivate state lifecycle differs). A no-op `DataPost` override on the stacker (`e35d21c`) was harmless but did not fix the deadlock. Two follow-up directions: (a) read `src/libmodelbox/engine/match_stream.cc` for the STREAM-vs-NORMAL EOF reconciliation rules and emit whatever termination signal the engine expects from the stacker; or (b) restructure the topology so `source_frame` flows through the stacker (e.g. as a passthrough output port) and `tracknet_post` becomes a single-input flowunit. Until fixed, the runner produces a usable but truncated MP4 — visually adequate to verify the ball circle.

- **Per-flowunit TOMLs dropped.** The plan called for `tracknet_frame_stacker.toml` and `tracknet_post.toml`. Implementation dropped both (`e0c27de`) once it was confirmed that C++ flowunits in this codebase register via the `MODELBOX_DRIVER_FLOWUNIT` macro inside their `.dylib`/`.so`, and the driver loader globs both extensions (see `src/libmodelbox/base/drivers/driver.cc:902-904`). Only the virtual `tracknet_deep` Core ML wrapper ships a TOML — that one is mandatory because the engine needs `virtual_type = "coreml"` + the `.mlpackage` entry to dispatch to `coreml_inference`.
