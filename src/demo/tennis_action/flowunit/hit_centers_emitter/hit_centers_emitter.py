...
def intervals_to_centers(logits: np.ndarray, video_fps: float,
    ...
    A hit's audio-time center is the midpoint of [t_start, t_end], where
    t_start = run_start * step_sec and t_end = run_end * step_sec + window_sec.
    """
...
def _maybe_add_center(logits, run_start, run_end, step_sec, window_sec,
    ...
    # Duration = span of run start-times (matches hit_intervals_sink C++).
    # Excludes the trailing window_sec so a 1.0 s run isn't dropped by a
    # max_hit_dur=1.0 config.
    if (run_end - run_start) * step_sec > max_hit_dur:
        return
    ...
    def process(self, data_context):
        ...
        for buf in logits_in:
            ...
            if arr.size == 0 or arr.size % 2 != 0:
                modelbox.error(
                    f"hit_centers_emitter: bad logit buffer size {arr.size}; "
                    "expected even multiple of 2 (per-window [no-hit, hit] logits)")
