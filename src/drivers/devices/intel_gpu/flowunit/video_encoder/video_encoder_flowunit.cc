...
modelbox::Status VideoEncoderFlowUnit::Open(
  ...
  // Default to the Intel QSV H.264 encoder. The user can override with any
  // FFmpeg encoder name via `encoder=...` in the flow unit config.
  encoder_name_ = opts->GetString("encoder", "h264_qsv");
  MBLOG_INFO << "intel_gpu video_encoder: using encoder '" << encoder_name_
             << "'";
...
modelbox::Status VideoEncoderFlowUnit::ReadFrames(
  ...
  for (auto frame_buffer : *frame_buffer_list) {
    ...
    std::shared_ptr<AVFrame> nv12_frame;
    ret = CvtFrameToYUV420P(color_cvt, av_frame, nv12_frame);
    if (ret != modelbox::STATUS_SUCCESS) {
      MBLOG_ERROR << "Convert frame to NV12 failed";
      return ret;
    }

    av_frame_list.push_back(nv12_frame);
  }
...
modelbox::Status VideoEncoderFlowUnit::CvtFrameToYUV420P(
    ...
    std::shared_ptr<AVFrame> &nv12_frame) {
  // Note: function name retained for header compatibility; output is NV12,
  // which is what the Intel QSV / VAAPI encoders expect.
  ...
  nv12_frame.reset(frame, [](AVFrame *ptr) {
    av_freep(&ptr->data[0]);
    av_frame_free(&ptr);
  });
  nv12_frame->width = origin->width;
  nv12_frame->height = origin->height;
  nv12_frame->format = AVPixelFormat::AV_PIX_FMT_NV12;
  nv12_frame->pts = origin->pts;
  auto ffmepg_ret = av_image_alloc(nv12_frame->data, nv12_frame->linesize,
                                   nv12_frame->width, nv12_frame->height,
                                   AVPixelFormat::AV_PIX_FMT_NV12, 1);
  if (ffmepg_ret < 0) {
    GET_FFMPEG_ERR(ffmepg_ret, ffmpeg_err);
    MBLOG_ERROR << "av_image_alloc failed, width " << nv12_frame->width
                << ",height " << nv12_frame->height << ",err " << ffmpeg_err;
    return modelbox::STATUS_FAULT;
  }

  auto ret = color_cvt->CvtColor(origin, nv12_frame->data[0],
                                 AVPixelFormat::AV_PIX_FMT_NV12);
  if (ret != modelbox::STATUS_SUCCESS) {
    MBLOG_ERROR << "Convert color failed";
    return ret;
  }
