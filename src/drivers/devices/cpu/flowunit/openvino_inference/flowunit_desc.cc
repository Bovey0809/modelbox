...
#include "openvino_cpu_inference_flowunit.h"

constexpr const char *FLOWUNIT_NAME = "openvino_inference";
constexpr const char *FLOWUNIT_DESC = "A cpu OpenVINO inference flowunit";

std::shared_ptr<modelbox::DriverFactory> CreateDriverFactory() {
  std::shared_ptr<modelbox::DriverFactory> factory =
      std::make_shared<OpenVINOCpuFlowUnitFactory>();
  return factory;
}
