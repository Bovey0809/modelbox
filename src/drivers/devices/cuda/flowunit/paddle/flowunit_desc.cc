...
constexpr const char *FLOWUNIT_NAME = "paddle_inference";
constexpr const char *FLOWUNIT_DESC =
    "A Paddle Inference (GPU) flowunit. Loads .pdmodel + .pdiparams via "
    "Paddle Inference C++ on CUDA.";

std::shared_ptr<modelbox::DriverFactory> CreateDriverFactory() {
  std::shared_ptr<modelbox::DriverFactory> factory =
      std::make_shared<PaddleInferenceFlowUnitFactory>();
  return factory;
}
